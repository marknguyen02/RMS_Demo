...
class TransactionController {
    ...
    public function processRequest()
    {
        switch ($this->requestMethod) {
            case 'GET':
                ...
                break;
            case 'POST':
                $response = $this->createTransactionFromRequest($_POST);
                break;
            default:
    ...
    ***/
    private function createTransactionFromRequest(Array $input)
    {
    ...
    ***/
    private function validatetransaction(Array $input)
    {
        $name = $input['name'];
        $amount = $input['amount'];
        ...
        if(!isset($name) || strlen($name) < 3 || strlen($name) > 255 || preg_match('/[^a-z0-9]+/i', $name))
            return false;
        else if($type !== "BET" && $type !== "WIN")
            return false;
        else if(!intval($amount) || ($type == "BET" && intval($amount) > 0) || ($type == "WIN" && intval($amount) < 0))
            return false;
        else if(substr($reference, 0, 3) !== 'TR-' || strlen($reference) < 3 || strlen($reference) > 255)
            return false;
